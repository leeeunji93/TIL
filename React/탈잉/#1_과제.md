### 비밀번호 검증하기
Q1. 6자리 이상이 아닐 경우 
`<button disabled={password.length < 6}>`

Q2. 비밀번호 길이가 6 이상,username을 포함하고 있지 않음,알파벳 대소문자 각각 하나 이상<br>
조건 1. 유효성 검사를 통과해야 제출 버튼 활성화

```javascript
import React,{Component} from "react";

class App extends Component{
    id = 1;
    state = {
        username : '',
        password : '',
        list : []
    };

    handleChange = e =>{
        const { value,name } = e.target;
        this.setState({
            [name] : value
        });
    };

    handleInsert = () => {
        const { username,password,list } = this.state;
        this.setState({
            username: '',
            password: '',
            list:list.concat({id:this.id,username,password})
        });
        this.id = +1;
    };

    inSafe = (username,password) => {
        const patten = /^(?=\w{6,}$)(?=.*[a-z])(?=.*[A-Z])/;
        if(!patten.test(password)){
            return {
                safe: false,
                msg: "대소문자를 넣어주세요!"
            };
        }

        if(password.search(username) > -1) {
            return {
                safe: false,
                msg: "유저명이 포함되어있음!"
            };
        }
        return {safe: true, msg:"ok"};
    };
    
    render(){
        const { username,password,list } = this.state;
        const {safe, msg} = this.inSafe(username,password);
        return(
         <div>
             <input
                 name = "username"
                 placeholder= "이름"
                 value = {username}
                 onChange = {this.handleChange}
             />
             <input
                 name = "password"
                 placeholder= "비밀번호"
                 value = {password}
                 onChange = {this.handleChange}
             />
             <button
                 onClick={this.handleInsert}
                 disabled={!safe}
             > {safe ? "submit" : msg}
                 </button>
             <ul>
                 {list.map((item) =>(
                     <li key = {item.id}>
                         {item.username} {item.password}</li>
                 ))}
             </ul>

         </div>
        )
    }
}

export default App;
```

조건2<br>
1. 제출버튼은 무조건 활성화<br>
2. 제출버튼을 누르고 나면 유효성검사<br>
3. 유효성검사할때 에러있으면 alert 띄워주기<br>

```javascript
import React,{Component} from "react";

class App extends Component{
    id = 1;
    state = {
        username : '',
        password : '',
        list : []
    };

    handleChange = e =>{
        const { value,name } = e.target;
        this.setState({
            [name] : value
        });
    };

    handleInsert = () => {
        const { username,password,list } = this.state;
        const {safe, msg} = this.inSafe(username,password)
        if(safe === false) {
            alert(msg)
            return
        }

        this.setState({
            username: '',
            password: '',
            list:list.concat({id:this.id,username,password})
        });
        this.id = +1;
    };

    inSafe = (username,password) => {
        const patten = /^(?=\w{6,}$)(?=.*[a-z])(?=.*[A-Z])/;
        if(!patten.test(password)){
            return {
                safe: false,
                msg: "대소문자를 넣어주세요!"
            };
        }

        if(password.search(username) > -1) {
            return {
                safe: false,
                msg: "유저명이 포함되어있음!"
            };
        }
        return {safe:
                true,
            msg:"ok"};
    };

    render(){
        const { username,password,list } = this.state;
        const {safe, msg} = this.inSafe(username,password);
        return(
         <div>
             <input
                 name = "username"
                 placeholder= "이름"
                 value = {username}
                 onChange = {this.handleChange}
             />
             <input
                 name ="password"
                 placeholder ="비밀번호"
                 value = {password}
                 onChange = {this.handleChange}
             />
             <button
                 onClick={this.handleInsert}
             > submit
                 </button>
             <ul>
                 {list.map((item) =>(
                     <li key = {item.id}>
                         {item.username} {item.password}</li>
                 ))}
             </ul>

         </div>
        )
    }
}

export default App;
```

조건 3<br>
1. 제출버튼은 유효성이 통과될때만 활성화된다<br>
2. 유효성이 통과되지 않을때는 그 이유를 화면에 표시해줘야 한다<br>
2-1 유효성검사 함수를 만든다<br>
2-2 잘못된게 있으면, 에러 메세지를 return 해준다<br>
2-3 잘못된게 없으면, 에러가 없으므로 null 을 리턴해준다<br>

```javascript
import React,{Component} from "react";

class App extends Component{
    id = 1;
    state = {
        username : '',
        password : '',
        list : []
    };

    handleChange = e =>{
        const { value,name } = e.target;
        this.setState({
            [name] : value
        });
    };

    handleInsert = () => {
        const { username,password,list } = this.state;

        const error = this.inSafe(username,password)
          //같음 == 같음 연산자는 두 피연산자의 값이 같으면 true / 같지 않음!= 피연산자듸 값이 같지 않으면 true  
        if(error != null) {
            alert(error)
            return
        }

        this.setState({
            username: '',
            password: '',
            list:list.concat({id:this.id,username,password})
        });
        this.id = +1;
    };

    inSafe = (username,password) => {
        const patten = /^(?=\w{6,}$)(?=.*[a-z])(?=.*[A-Z])/;
        if(!patten.test(password)){
            return "대소문자를 넣어주세요!"
        }

        if(password.search(username) > -1) {
            return "유저명이 포함되어있음!"
        }
        
        return null
    };


    render(){
        const { username,password,list } = this.state;
        // const {safe, msg} = this.inSafe(usernamejxms,password);
        return(
         <div>
             <input
                 name = "username"
                 placeholder= "이름"
                 value = {username}
                 onChange = {this.handleChange}
             />
             <input
                 name ="password"
                 placeholder ="비밀번호"
                 value = {password}
                 onChange = {this.handleChange}
             />
             <button onClick={this.handleInsert}>submit</button>
             
             <ul>
                 {list.map((item) =>(
                     <li key = {item.id}>
                         {item.username} {item.password}</li>
                 ))}
             </ul>

         </div>
        )
    }
}

export default App;
```
조건4<br>
1.이멜 유효성 검사를 간편하게 해주자.
```javascript
 import React, {Component} from "react";

class App extends Component {
    id = 1;
    state = {
        username: '',
        password: '',
        email: '',
        list: []
    };

    handleChange = e => {
        const {value, name} = e.target;
        this.setState({
            [name]: value
        });
    };

    handleInsert = () => {
        const {username, password, email, list} = this.state;
        this.setState({
            username: '',
            password: '',
            email:"",
            list: list.concat({id: this.id, username, password,email})
        });
        this.id = ++;
    };



    isPasswordSafe = (password,username) => {
        const patten = /^(?=\w{6,}$)(?=.*[a-z])(?=.*[A-Z])/;
        if (!patten.test(password)) {
            return {
                safe: false,
                msg: "대소문자를 넣어주세요!"
            };
        }
        if(password.search(username) > -1) {
            return {
                safe: false,
                msg: "유저명이 포함되어있음!"
            };
        }
    }

    isEmailSafe = (email) => {

        return {
            safe: false,
            msg: "@ 넣어주세요!"
        };
    }

    isSafe = () => {
        const { username,password,email,list } = this.state;
        pwdsafe = isPasswordSafe(password,usename).safe;
        emailsafe = isEmailSafe(username).safe;
        return pwdsafe && emailsafe;
    }


    /*inSafe = (username, password, email) => {
        const patten = /^(?=\w{6,}$)(?=.*[a-z])(?=.*[A-Z])/;
        if (!patten.test(password)) {
            return {
                safe: false,
                msg: "대소문자를 넣어주세요!"
            };
        }

        if (password.search(username) > -1) {
            return {
                safe: false,
                msg: "유저명이 포함되어있음!"
            };
        }
        if (!email.includes("@")) {
            return {
                safe: false,
                msg: "@ 넣어주세요!"
            };
        }
        return {safe: true, msg: "ok"};
    };
*/
    render() {
        const {username, password,email, list} = this.state;
        const {safe, msg} = this.isSafe();
        return (
            <div>
                <div>
                <input
                    name="username"
                    placeholder="이름"
                    value={username}
                    onChange={this.handleChange}
                />
                </div>

                <div>
                <input
                    name="password"
                    placeholder="비밀번호"
                    value={password}
                    onChange={this.handleChange}
                />
                </div>

                <div>
                <input
                    name="email"
                    placeholder="이메일"
                    value={email}
                    onChange={this.handleChange}
                />
                </div>

                <button
                    onClick={this.handleInsert}
                    disabled={!safe}
                > {safe ? "submit" : msg}
                </button>
                <ul>
                    {list.map((item) => (
                        <li key={item.id}>
                          <div>{item.username}</div>
                            <div>{item.password}</div>
                            <div> {item.email}</div>
                        </li>
                    ))}
                </ul>

            </div>
        )
    }
}

export default App;
```

- `disabled` : bollen 타입으로 `true` 받으면 활성화,  `false` 받으면 비활성화<br>
여기선 safe값에 따라 바뀌는건데, {!safe} 는 safe값이 true가 아니면 비활성화 하겠다! 
